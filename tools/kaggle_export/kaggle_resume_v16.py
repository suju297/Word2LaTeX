# Kaggle Training Script - Resume V16 (Continue from V23 Epoch 37 → 50)
# ==================================================================
# This script resumes training from the version_23 checkpoint
# Target: Complete 13 more epochs (37 → 50 total)
#
# INSTRUCTIONS:
# 1. Upload version_23/runs/detect/train_v5_stage_b_resume as a Dataset:
#    - Name it "v23-checkpoint" (slug: v23-checkpoint)
#    - Structure: v23-checkpoint/train_v5_stage_b_resume/weights/last.pt
# 2. Add these datasets to your notebook:
#    - v23-checkpoint (your checkpoint)
...
# Check if ultralytics is already installed (Kaggle has it pre-installed)
try:
    import ultralytics
...
RESUME_RUN_DIR_PATHS = [
    "/kaggle/input/version23-checkpoint/train_v5_stage_b_resume",
    "/kaggle/input/version23-checkpoint/runs/detect/train_v5_stage_b_resume",
    "/kaggle/input/version23-checkpoint",
]
...
RESUME_WEIGHTS_PATHS = [
    ...
    "/kaggle/input/version23-checkpoint/runs/detect/train_v5_stage_b_resume/weights/last.pt",
    "/kaggle/input/version23-checkpoint/weights/last.pt",
]

PREVIOUS_EPOCHS_COMPLETED = 37  # From version_23 train_v5_stage_b_resume
TARGET_TOTAL_EPOCHS = 50        # Target total - 13 more epochs
USE_TRUE_RESUME = False         # Use weights-only resume (more reliable, epochs show as 1/13, 2/13...)

def sanitize_checkpoint_for_ddp(ckpt_path: str, target_epochs: int = 50) -> str:
    """
    Strip optimizer state from checkpoint to avoid DDP mismatch errors.
    Also update epochs in train_args to allow continuing training.
    ...
    # CRITICAL: Update epochs in train_args to allow continuing training
    # YOLO reads this value, not args.yaml!
    if "train_args" in ckpt and isinstance(ckpt["train_args"], dict):
        old_epochs = ckpt["train_args"].get("epochs", "?")
        ckpt["train_args"]["epochs"] = target_epochs
        print(f"  - Updated train_args.epochs: {old_epochs} → {target_epochs}")
...
if can_true_resume:
    ...
    target_run_dir = os.path.join(project_dir, "train_v5_stage_b_resume")
    ...
    # CRITICAL: Update args.yaml to set new target epochs
    # Without this, YOLO thinks training to 37 is finished
    args_yaml_path = os.path.join(target_run_dir, "args.yaml")
    if os.path.exists(args_yaml_path):
        print(f"Updating args.yaml to set epochs={TARGET_TOTAL_EPOCHS}...")
        with open(args_yaml_path, 'r') as f:
            args_config = yaml.safe_load(f)
        args_config['epochs'] = TARGET_TOTAL_EPOCHS
        with open(args_yaml_path, 'w') as f:
            yaml.safe_dump(args_config, f, default_flow_style=False)
        print(f"  - Updated epochs: 37 → {TARGET_TOTAL_EPOCHS}")
    
    # CRITICAL: Sanitize checkpoint to remove optimizer state (causes DDP mismatch)
    working_ckpt = sanitize_checkpoint_for_ddp(working_ckpt, TARGET_TOTAL_EPOCHS)
    ...
    model.train(
        resume=True,  # TRUE RESUME - epochs continue as 37/50, 38/50, etc.
        device=device_ids,
    )
    
    run_name = "train_v5_stage_b_resume"  # Same name since we're continuing
    ...
    log_kv([
        ...
        ("note", f"Model weights from epoch {PREVIOUS_EPOCHS_COMPLETED}, but showing 1/N"),
        ("model", "YOLO11s (Small)"),
        ("device", device_ids),
    ])
    
    # Copy checkpoint to a stable path for DDP access
    working_ckpt = Path("/kaggle/working") / "last_from_v23.pt"
    ...
    run_name = "train_v5_stage_b_resume2"
...
print("\n📦 Zipping outputs...")
os.system(
    'zip -r -1 -q results_v16.zip /kaggle/working/runs '
    '-x "/kaggle/working/data/*"'
)
print("✅ Download 'results_v16.zip' from Output.")
